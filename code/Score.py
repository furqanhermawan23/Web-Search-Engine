...
with open('../data/index/index.txt', 'w') as file:
    for term, freq in tqdm(tf.items()):
        ...
        for doc, tfdoc in df.items():
            if term in tfdoc:
                # bobot paling tinggi title, lalu mains, lalu bagian yang lain
                if term in titles[doc]:
                    weight = (tfdoc[term] * idf[term])*3/4
                elif term in mains[doc]:
                    weight = (tfdoc[term] * idf[term])*2/4
                else:
                    weight = (tfdoc[term] * idf[term])*1/4
            else:
                file.write(f' {doc}:0')
                continue
            file.write(f' {doc}:{weight}')
        file.write('\n')
print('done')
